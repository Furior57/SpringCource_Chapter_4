...
public class Lesson_53_Rest_api_meeting {
    // Начинаем новую, обширную тему связанную со способами передачи и обработки информации
    ...
    // определенному url, где определенны определенные правила и условия.
    ...
    // функции друг друга удаленно. Так же API должно учитывать, что программы могут быть написаны на
    // разных языках программирования.

    // REST, по определению его создателя(кстати, он же создатель HTTP, что определяет
    // предрасположенность стиля REST к работе с этим протоколом) при правильно проектировании
    ...
    // хранить у себя информацию о сессии с клиентом(однако не всегда, кеширование часто используется).
    ...
    // увеличение нагрузки на сеть, каждый раз мы передаем всю информацию.
    // 3) Кеширование - этот подход позволяет избегать однотипных запросов. Представим ситуацию,
    // когда наш сервер получив запрос обращается для ответа к третьему серверу, если подобных
